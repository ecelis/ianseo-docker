...
$q=safe_r_sql("SELECT * FROM Events 
  ...
  WHERE EvTournament=" . StrSafe_DB($_SESSION['TourId']) . " AND EvTeamEvent='1' and EvCode=".StrSafe_DB($_REQUEST['Event'])."
...
echo '<option value="0"'.($EVENT->EvElimType==0 ? ' selected="selected"' : '').'>'.get_text('RegularBrackets', 'RoundRobin').'</option>';
...
echo '<option value="5"'.($EVENT->EvElimType==5 ? ' selected="selected"' : '').'>'.get_text('R-Session', 'Tournament').'</option>';
echo '';
echo '</select>';
echo '</td>';
echo '</tr>';
