 ...
 * )
 */
	class Obj_Rank_FinalTeam_3_SetFRD12023 extends Obj_Rank
	{
	/**
	 * safeFilterR()
				...
				while ($myRow=safe_fetch($r)) {
					if ($myEv!=$myRow->TeEvent) {
						...
						$fields = array(
							...
							'qualScore' => get_text('Points', 'Tournament'),
							'qualGolds' => get_text('TieBreak-1', 'RoundRobin'),
							'qualXnine' => get_text('TieBreak-2', 'RoundRobin'),
					...
					if ($myTeam!=$myRow->CoId . $myRow->TeSubTeam . $myRow->TeEvent) {
                        $item=array(
                            ...
                            'qualGolds'		=> $myRow->TeGold,
                            'qualXnine'		=> $myRow->TeXnine,
