...
if(!empty($rankData['TeamsFinalRank'])) {
	$FromRobin=true;
	$OldRankData=$rankData;
	$PdfData->rankData=$rankData['TeamsFinalRank'];
	include(__DIR__.'/RankTeam_3_SetFRD12023.inc.php');

	// $pdf->AddPage();
	$PdfData->rankData=$OldRankData;
}
...
if(count($rankData['sections'])) {
	...
	foreach($rankData['sections'] as $IdEvent => $section) {
		...
		if($pdf->ShowRank and empty($rankData['TeamsFinalRank'])) {
			...
			foreach($section['levels'] as $idLevel => $level) {
				...
				$colW1=15;
				$colW2=20;
				$colW3=($pdf->getPageWidth()-20-($colW1*4)-($colW2*($level['tiebreaker2']?3:2)));
				...
				foreach($level['ranks'] as $idGroup => $group) {
					...
					foreach($group['items'] as $item) {
						...
						$pdf->cell($colW2, $CellHeight, $item['countryName'],'1',0, 'L');
		...
		if($pdf->ShowMatches) {
			...
			foreach($section['levels'] as $idLevel => $level) {
				...
				$colW2=12;
				$colW3=10;
				$colW4=($MatchWidth-$colW1-$colW3*($level['tiesAllowed'] ? 2 : 3)-$colW2);
				foreach ($level['matches'] as $idGroup => $group) {
					...
					foreach ($group['rounds'] as $round) {
						if($OldPlace!=$round['place']) {
							$pdf->dy(2);
							$pdf->SetFont('', 'b', 10);
							$pdf->cell(0, 0, $round['place'], '1', 0, 'C', '1');
							$pdf->SetFont('', '', 8);
							$pdf->ln(5);
						...
						$pdf->cell($colW1, $CellHeight, $section['meta']['fields']['rank'], '1', 0, 'C', '1');
						$pdf->cell($colW4, $CellHeight, $section['meta']['fields']['athlete'], '1', 0, 'L', '1');
						$pdf->cell($colW2, $CellHeight, $section['meta']['fields']['countryName'], '1', 0, 'L', '1');
						$pdf->cell($colW3, $CellHeight, $section['meta']['fields']['score'], '1', 0, 'C', '1');
						if(!$level['tiesAllowed']) {
							$pdf->cell($colW3, $CellHeight, $section['meta']['fields']['so'], '1', 0, 'C', '1');
						}
						$pdf->cell($colW3, $CellHeight, $section['meta']['fields']['points'], '1', 0, 'C', '1');
						...
						foreach ($round['items'] as $item) {
							if($item['athlete'] or $item['oppAthlete']) {
								...
								$pdf->cell($colW4, $CellHeight, $item['athlete'] ?: $rankData['meta']['bye'] , '1', 0, 'L');
								$pdf->cell($colW2, $CellHeight, $item['countryCode'] ?:'', '1', 0, 'L');
								if($item['irm']) {
									$pdf->cell($colW3*($level['tiesAllowed'] ? 2 : 3), $CellHeight, $item['irmText'], '1', 0, 'L');
									$pdf->SetFont('', '', 8);
								} elseif($print) {
									if($item['athlete']) {
										$pdf->cell($colW3, $CellHeight, $item[$AthScoreField], '1', 0, 'C');
										if(!$level['tiesAllowed']) {
											$pdf->cell($colW3, $CellHeight, $item['tiebreakDecoded'], '1', 0, 'C');
										}
										$pdf->cell($colW3, $CellHeight, $item['points'], '1', 0, 'C');
										// $pdf->cell($colW2, $CellHeight, $item['tieBreaker'], '1', 0, 'C');
										// $pdf->cell($colW2, $CellHeight, $item['tieBreaker2'], '1', 0, 'C');
									} else {
										$pdf->cell($colW3*($level['tiesAllowed'] ? 2 : 3), $CellHeight, '', '1', 0, 'L');
									}
								} else {
									$pdf->SetFont('', 'i', 7);
									$pdf->cell($colW3*($level['tiesAllowed'] ? 2 : 3), $CellHeight*2, $item['scheduledDate'].' @ '.$item['scheduledTime'], '1', 0, 'L');
									$pdf->SetFont('', '', 8);
								}
								$pdf->ln($CellHeight);
								if($item['oppIrm']) {
								...
								$pdf->cell($colW4, $CellHeight, $item['oppAthlete'] ?: $rankData['meta']['bye'], '1', 0, 'L');
								$pdf->cell($colW2, $CellHeight, $item['oppCountryCode'] ?: '' , '1', 0, 'L');
								if($item['oppIrm']) {
									$pdf->cell($colW3*($level['tiesAllowed'] ? 2 : 3), $CellHeight, $item['oppIrmText'], '1', 0, 'L');
									$pdf->SetFont('', '', 8);
								} elseif($print) {
									if($item['oppAthlete']) {
										$pdf->cell($colW3, $CellHeight, $item[$OppScoreField], '1', 0, 'C');
										if(!$level['tiesAllowed']) {
											$pdf->cell($colW3, $CellHeight, $item['oppTiebreakDecoded'], '1', 0, 'C');
										}
										$pdf->cell($colW3, $CellHeight, $item['oppPoints'], '1', 0, 'C');
								...
								if($item['scheduledDate']) {
									$txt=$item['scheduledDate'];
								}
								if($item['scheduledTime']) {
									$txt.=' @ '.$item['scheduledTime'];
								}
								$pdf->cell($colW1, $CellHeight, $item['target'], '1', 0, 'C');
								$pdf->cell($colW4, $CellHeight, '', '1', 0, 'L');
								$pdf->cell($colW2, $CellHeight, '', '1', 0, 'L');
								$pdf->cell($colW3*($level['tiesAllowed'] ? 2 : 3), $CellHeight*2, $txt, '1', 0, 'L');
								$pdf->ln($CellHeight);
								$pdf->cell($colW1, $CellHeight, $item['oppTarget'], '1', 0, 'C');
								$pdf->cell($colW4, $CellHeight, '', '1', 0, 'L');
								$pdf->cell($colW2, $CellHeight, '', '1', 0, 'L');
