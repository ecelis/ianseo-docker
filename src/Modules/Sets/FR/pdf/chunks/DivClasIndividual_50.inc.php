	...
	foreach($rankData['sections'] as $section)
	{
		...
		if($section['meta']['numDist']>=4) {
			...
			if(!$rankData['meta']['double']) {
				$DistSize = (40 + ($hideGolds ? 10 : 0))/$section['meta']['numDist'];
			} else {
				$DistSize = (40 + ($hideGolds ? 10 : 0))/(($section['meta']['numDist']/2)+1);
			}
		} else {
			$AddSize = ((40 + ($hideGolds ? 10 : 0))-($section['meta']['numDist']*(48 + ($hideGolds ? 10 : 0))/4))/2;
		}
...
function writeDataRowPrnIndividual($pdf, $item, $distSize, $addSize, $distances, $double, $snapDistance, $HideGolds=false)
{
	...
	else
	{
  	...
  	if($snapDistance==0) {
		$pdf->Cell(8, 4 * ($double ? 2 : 1), is_numeric($item['score']) ? number_format($item['hits'] - $item['miss'], 0, '', $pdf->NumberThousandsSeparator) : '', 1, 0, 'R', 0);
